...
def send_batch_data(stop_event):
    ...
    sys_logger.info("🚀 Upload service started (Production Mode)")
    ...
    while not stop_event.is_set():
        try:
            ...
            if failed_ids:
                sys_logger.warning(f"⚠️ {len(failed_ids)} records failed to decode")
                ...
                if repeated_failed_ids:
                    sys_logger.warning(f"🚨 {len(repeated_failed_ids)} records repeatedly failed, marking as permanently failed")
                    try:
                        mark_data_as_failed(repeated_failed_ids, 'repeated_decode_failure')
            ...
            if not decoded_list:
                sys_logger.info(f"🚀 Attempting to send {len(decoded_list)} decoded records to API")
    ...
    sys_logger.info("🚀 Upload service stopped")
    ...
    for record in data:
        ...
        try:
            ...
            if decoded:
                ...
                # Log failed decode untuk debugging
                if len(failed_ids) <= 3:
...
def send_to_api(data, ids, stop_event):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            sys_logger.info(f"📡 API Response: Status {response.status_code}")
